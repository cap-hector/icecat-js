...
/**
 * getLongSummaryDescription
 * @returns {string}
 */
icecat.prototype.getLongSummaryDescription = function() {
  try {
...
icecat.prototype.getImages = function() {
  const hasImages = (this.productData.ProductGallery || []).filter(Boolean).length;
  this.productData.ProductGallery = hasImages ? this.productData.ProductGallery : [{}];
  if (
    'ProductPicture' in this.productData.ProductGallery[0] &&
    this.productData.ProductGallery[0].ProductPicture.length
  ) {
    return this.productData.ProductGallery[0].ProductPicture.map((img) => {
      return {
        IsMain: img.$.IsMain || 'N',
        Id: img.$.ProductPicture_ID,
        Type: img.$.Type,
        Original: img.$.Original,
        HighImg: img.$.Pic,
        LowImg: img.$.LowPic,
        MediumImg: img.$.Pic500x500,
        ThumbPic: img.$.ThumbPic
      };
    });
  }
  return [];
...
icecat.prototype.getSpecifications = function() {
  this.productData.ProductFeature = this.productData.ProductFeature || [];
