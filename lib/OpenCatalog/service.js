...
openCatalog.prototype._getBaseUrl = function(lang) {
  if (this.icecat.apiToken) {
    return `${this.icecat.scheme}${this.icecat.httpUrl}?lang=${lang};output=productxml`;
  }
...
openCatalog.prototype._requestProduct = function(httpRequestUrl) {
  return new Promise((resolve, reject) => {
    const requestOptions = {
      headers: this.icecat.apiToken ? { 'Api-Token': this.icecat.apiToken } : {},
    };
    const request = this.https.get(httpRequestUrl, requestOptions, (response) => {
      let body = '';
